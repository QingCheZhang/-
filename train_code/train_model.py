...
model_m.compile(loss=categorical_crossentropy_2d, optimizer=optimizer1)
...
for epoch_num in range(EPOCH_NUM):
    ...
    idx = idx_train[(epoch_num+1) % len(idx_train)]
    X = h5f['X' + str(idx)][:]
    Y = h5f['Y' + str(idx)][:]
    
    Xc, Yc = clip_datapoints(X, Y, CL, N_GPUS) 
    # print('Xc_len:', len(Xc), 'Xc_type:', type(Xc))
    # print('Yc_len:', len(Yc[0]), 'Xc_type:', type(Yc[0]))
    #tensorboard_callback = TensorBoard(log_dir='./logs', histogram_freq=1, write_graph=True, write_images=True)
    #model_m.fit(Xc, Yc, batch_size=BATCH_SIZE, verbose=0, callbacks=[tensorboard_callback])
    history = model_m.fit(Xc, Yc, batch_size=BATCH_SIZE, verbose=0)
    #if (epoch_num+1) % len(idx_train) == 0: 
    print("损失历史：", history.history['loss'])

    # 查看训练过程中的准确度历史
    #print("准确度历史：", history.history['accuracy'])    
